...
abstract contract Delegation is Core {
    ...
    function undelegate() external {
        ...
        require(
            previous != address(0),
            "Governance: tokens are already undelegated"
        );
...
contract Governance is Initializable, Configuration, Delegation, EnsResolve {
    ...
    function propose(address target, string memory description)
        external
        returns (uint256)
    {
    ...
    function _propose(
        ...
        require(
            ...
            "Governance::propose: not a contract"
        );
        ...
        _lockTokens(
            proposer,
            endTime.add(VOTE_EXTEND_TIME).add(EXECUTION_EXPIRATION).add(
                EXECUTION_DELAY
            )
        );
    ...
    function execute(uint256 proposalId) external payable virtual {
        ...
        require(
            Address.isContract(target),
            "Governance::execute: not a contract"
        );
    ...
    function _castVote(
        ...
        if (
            !proposal.extended &&
            proposal.endTime.sub(getBlockTimestamp()) < CLOSING_PERIOD
        ) {
        ...
        _lockTokens(
            ...
            proposal
                .endTime
                .add(VOTE_EXTEND_TIME)
                .add(EXECUTION_EXPIRATION)
                .add(EXECUTION_DELAY)
        );
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        ...
        if (getBlockTimestamp() <= proposal.startTime) {
            ...
            getBlockTimestamp() >=
            proposal.endTime.add(EXECUTION_DELAY).add(EXECUTION_EXPIRATION)
        ) {
            return ProposalState.Expired;
        } else if (
            getBlockTimestamp() >= proposal.endTime.add(EXECUTION_DELAY)
        ) {
